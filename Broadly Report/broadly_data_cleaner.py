...
LLM_MODEL = "phi3:mini"  # Ollama model - faster for data cleaning tasks
...
ANALYSIS PROCESS:
1. FIRST examine Contact Name - if it's a clear person name (not company), prioritize it
2. CHECK Customer Name - only use if it's clearly a person name, not a company
...
- If Contact Name provides first name, use email to find matching last name

MULTIPLE EMAIL HANDLING:
- Choose the email most likely to be a person (not info@, sales@, etc.)
...
- Company indicators: LLC, INC, CORP, GROUP, CENTER, CATERING, ENTERTAINMENT, RESTAURANT, etc.
- If only company names available and no extractable person name from email, return null
